...
"""
GRIB2 generator for Copernicus Marine (CMEMS) Mediterranean surface currents.
...
identical wire format and identical pipeline shape to
generate_ibi_gribs.py/generate_nws_gribs.py. See sources/cmems_med.py's
module docstring for why: the MED
`cmems_mod_med_phy-cur_anfc_4.2km-2D_PT1H-m` dataset is, like IBI and NWS, a
plain regular lat/lon grid with 1-D lat/lon coordinates and no depth
dimension — so no pooling, no curvilinear regridding needed.
...
.github/workflows/generate-med-gribs.yml and sources/cmems_med.py, which
owns the static catalog entry and is run weekly by generate_index.py).

Usage:
    python generate_med_gribs.py [--output-dir DIR] [--regions r1,r2] [--forecast-hours N]
...
# sources/cmems_med.py is the single source of truth for the CMEMS MED
...
import cmems_med  # noqa: E402

REGIONS = cmems_med.REGIONS
DATASET_ID = cmems_med.DATASET_ID
...
def process_region(rid, r, out_dir, forecast_hours_override=None):
    ...
    # continuous time series (analysis + forecast merged, same as IBI/NWS),
    # so every requested hour is a forward step from this reference;
...
def main():
    ...
    parser.add_argument("--output-dir", default="/tmp/med_gribs")
    parser.add_argument(
