...
"""
GRIB2 generator for Copernicus Marine (CMEMS) NWS surface currents.
...
multi-message GRIB2 file per region using the shared grib2_writer tail —
identical wire format and identical pipeline shape to generate_ibi_gribs.py.
See sources/cmems_nws.py's module docstring for why: the NWS
`cmems_mod_nws_phy-cur_anfc_1.5km-2D_PT1H-i` dataset is, like IBI, a plain
regular lat/lon grid with 1-D lat/lon coordinates and no depth dimension —
so no pooling, no curvilinear regridding needed, just non-square
lat/lon increments (NWS is at higher latitude than IBI, so a degree of
longitude is physically shorter there).
...
.github/workflows/generate-nws-gribs.yml and sources/cmems_nws.py, which
owns the static catalog entry and is run weekly by generate_index.py).

Usage:
    python generate_nws_gribs.py [--output-dir DIR] [--regions r1,r2] [--forecast-hours N]
...
# sources/cmems_nws.py is the single source of truth for the CMEMS NWS
...
# as generate_ibi_gribs.py / sources/cmems_ibi.py).
...
import cmems_nws  # noqa: E402

REGIONS = cmems_nws.REGIONS
DATASET_ID = cmems_nws.DATASET_ID
...
def process_region(rid, r, out_dir, forecast_hours_override=None):
    ...
    # continuous time series (analysis + forecast merged, same as IBI), so
    # every requested hour is a forward step from this reference;
    # out-of-range requests clip gracefully with a warning rather than
    # erroring.
    ...
    try:
        ...
        # Defensive ascending-order check, matching generate_ibi_gribs.py's
        # convention — confirmed ascending in practice, but don't assume it
        # forever.
...
def main():
    ...
    parser.add_argument("--output-dir", default="/tmp/nws_gribs")
    parser.add_argument(
